...
class ProtoNet:
    ...
    def _step(self, task_batch):
        ...
        for task in task_batch:
            ...
            N = len(set(labels_query.numpy()))

            images_query, labels_query = shuffle(images_query, labels_query)

            prot_t = self._network(images_support)
            prot = torch.stack([prot_t[labels_support==i].mean(0) for i in range(N)]).detach()

            pred = self._network(images_query)
            ...
            loss = F.cross_entropy(dist, labels_query)

            loss_batch.append(loss)
            
            with torch.no_grad():
                acc_q = util.score(dist.softmax(dim=1),labels_query)
                accuracy_query_batch.append(acc_q)

                dist1 = torch.zeros((prot_t.shape[0], prot.shape[0]))

                for i in range(len(labels_support)):
                    t = torch.zeros(prot.shape[0])
                    for j in range(len(prot)):
                        t[j] = torch.dist(prot_t[i],prot[j],2)
                    dist1[i] = -t

                acc_supp = util.score(dist1.softmax(dim=1),labels_support)
