...
class DataGenerator(object):
    ...
    def sample_batch(self, batch_type, batch_size):
        ...
        for i in range(batch_size):
            folder = np.random.choice(folders, n)
            one_hot_enc = F.one_hot(torch.arange(0,n),num_classes=n).numpy()
            data_train = get_images(folder,one_hot_enc,nb_samples=k,shuffle=False)
            data_test = get_images(folder,one_hot_enc,nb_samples=1,shuffle=True)

            for j in range(n):
                for l in range(k):
                    label,img = data_train[j*k + l]
                    img = image_file_to_array(img,784)
                    images[i,l,j] = torch.from_numpy(img)
                    labels[i,l,j] = torch.from_numpy(label)

                label,img = data_test[j]
                img = image_file_to_array(img,784)
